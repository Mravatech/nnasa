...
suspend fun <T, R> BroadcastChannel<T>.consumeTo(sendChannel: SendChannel<R>, mapper: (T) -> R = { it as R }) = consumeEach { sendChannel.send(mapper(it)) }
suspend fun <T, R> ConsumableChannel<T>.consumeTo(sendChannel: SendChannel<R>, mapper: (T) -> R = { it as R }) = consumeEach { sendChannel.send(mapper(it)) }