...
class ConvertersContextImpl : ConvertersContext {
    ...
    override fun <IN : Any, OUT : Any> convertCollection(collection: Iterable<IN>, token: Any?, outClass: Class<OUT>): List<OUT> {
        if (collection.none()) return emptyList()
        val firstElement = collection.firstOrNull { it != null } ?: return emptyList()
        val converter = getConverter(firstElement.javaClass, outClass)
        return collection.map { converter.invoke(it, token, this@ConvertersContextImpl) }
    }
