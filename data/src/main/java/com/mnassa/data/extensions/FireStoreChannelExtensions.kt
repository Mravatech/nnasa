...
internal suspend inline fun <reified DbType : HasId, reified OutType : HasId> CollectionReference.toAggregatorEvents(
    exceptionHandler: ExceptionHandler,
    noinline queryBuilder: (CollectionReference) -> Query = { it },
    noinline mapper: suspend (DbType) -> OutType? = { it as OutType },
    pagination: PaginationController? = null,
    limit: Int = DEFAULT_LIMIT
): Channel<AggregatorInEvent<OutType>> {
    forDebug { Timber.i("#LISTEN# toValueChannelWithChangesHandling ${this.path}") }
    val channel = Channel<AggregatorInEvent<OutType>>(limit)
    ...
    val doProcess: (List<DocumentChange>, Boolean, () -> Unit) -> Unit =
        { newChanges, isInit, onComplete ->
            // Add a snapshot to process to a
            // queue.
            runBlocking {
                queueMutex.withLock {
            ...
            rootCoroutineScope.launchWorker {
                ...
                workerMutex.withLock {
                    ...
                    val eventsBatchInit = if (isInit) {
                        LinkedList<AggregatorInEvent<OutType>>()
                    } else {
                        null
                    }
                    ...
                    for (changesBatch in changesBatches) {
                            ...
                            .map { documentChange ->
                                documentChange to asyncWorker {
                                    ...
                                    if (documentChange.type == DocumentChange.Type.REMOVED) {
                                        // Remove the model by it, instead of creating an
                                        // entity and then passing it to remove.
                                        return@asyncWorker AggregatorInEvent.Remove<OutType>(dbEntity.id)
                                    } else {
                                        // Map to out model
                                        val outEntity = try {
                                            coroutineScope {
                                            ...
                                            val path = documentChange.document.reference.path
                                            val msg =
                                                "Converting exception: class: ${OutType::class.java.name} id: $path"
                                            Timber.e(e, msg)
                                            null
                                        }

                                        return@asyncWorker if (outEntity == null) {
                                            AggregatorInEvent.Remove<OutType>(dbEntity.id)
                                        } else when (documentChange.type) {
                                            DocumentChange.Type.ADDED,
                                            DocumentChange.Type.MODIFIED ->
                                                AggregatorInEvent.Put(outEntity)
                                            else -> error("Unknown FireStore change type!")
                                        }
                                    }
                                }
                        ...
                        for (change in changesDeferred) {
                            ...
                            if (channel.isClosedForSend) {
                                ...
                                val event: AggregatorInEvent<OutType>? = change.second.await()
                                if (event != null) {
                                    if (eventsBatchInit != null) {
                                        eventsBatchInit.add(event)
                                    } else {
                                        try {
                                            channel.send(event)
                                        } catch (e: ClosedSendChannelException) {
                    ...
                    if (eventsBatchInit != null) {
                        try {
                            val event = eventsBatchInit
                                .mapNotNull { it as? AggregatorInEvent.Put<OutType> }
                                .let {
                                    AggregatorInEvent.Init(it)
                                }
                            channel.send(event)
                        } catch (e: ClosedSendChannelException) {
                            GlobalScope.launchUI {
    ...
    val doSubscribe: suspend (DocumentSnapshot?, DocumentSnapshot?, () -> Unit) -> Unit =
        { docStartAfter, docEndAt, onComplete ->
            rootCoroutineScope.launchUI {
                val registration = queryBuilder(this@toAggregatorEvents)
                    .run {
                        val query = docStartAfter?.let(::startAfter) ?: this
                        return@run docEndAt?.let(query::endAt) ?: query
                    }
                    .addSnapshotListener { dataSnapshot, e ->
                        ...
                        val changes = dataSnapshot?.documentChanges
                        if (changes != null) {
                            doProcess(changes, isFirstSubscription, onComplete)
                        }
            ...
            val observer = object : PaginationObserver {
                ...
                override fun onNextPageRequested(limit: Long) {
                    ...
                    rootCoroutineScope.launchWorker {
                        mutex.withLock {
                            val lastDoc = try {
                                val task = queryBuilder(this@toAggregatorEvents)
                                    .run {
                                        prevDoc?.let(::startAfter) ?: this
                                    }
                                    .limit(limit)
